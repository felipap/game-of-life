...
(function() {
  var clearGridSquare, fillGridSquare, getRandColor, rainbow,
  ...
  fillGridSquare = function(context, size, coord) {
    context.fillStyle = rainbow(coord.x + coord.y, 10);
    return context.fillRect(size * coord.x, size * coord.y, size, size);
  };

  clearGridSquare = function(context, size, coord) {
    return context.clearRect(size * coord.x, size * coord.y, size, size);
  };
  ...
  window.Board = (function() {
    ...
    Board.prototype.toogleSquare = function(coord) {
      if (this.boardState[coord.x][coord.y]) {
        clearGridSquare(this.context, this.gridSize, coord);
        return this.boardState[coord.x][coord.y] = this.DEAD;
      } else {
        fillGridSquare(this.context, this.gridSize, coord);
          ...
          _results.push((function() {
            ...
            for (y = _j = 0, _ref1 = newState[x].length; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; y = 0 <= _ref1 ? ++_j : --_j) {
              if (newState[x][y] !== this.boardState[x][y]) {
                if (!newState[x][y]) {
                  _results1.push(clearGridSquare(this.context, this.gridSize, {
                    x: x,
                    y: y
                  }));
                } else {
                  _results1.push(fillGridSquare(this.context, this.gridSize, {
                    x: x,
                    y: y
                  }));
                }
